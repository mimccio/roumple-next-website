...
export async function Header({ lang }: Props) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-10 flex h-14 items-center justify-end bg-gray-50 px-6 shadow-md 2xs:justify-between">
      <Link href="/" className="group hidden items-center gap-4 2xs:flex">
        <Logo />
      ...
      <a
        href={myRoumpleUrl}
        className="flex items-center rounded-lg border border-indigo-100 bg-indigo-50 px-5 py-1 font-semibold text-indigo-500 transition-colors duration-200 hover:border-indigo-200 hover:bg-indigo-100 hover:text-indigo-600"
      >
        {t('login', { ns: 'nav' })}
      </a>
    </header>
